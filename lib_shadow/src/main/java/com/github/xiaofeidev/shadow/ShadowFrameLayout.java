...
public class ShadowFrameLayout extends FrameLayout implements ShadowStatus {
    ...
    private void init(Context context, @Nullable AttributeSet attrs, int defStyleAttr){
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.ShadowFrameLayout,
                0, 0);

        try {
            float radius, topLeftRadius, topRightRadius, bottomLeftRadius, bottomRightRadius;

            radius = typedArray.getDimension(R.styleable.ShadowFrameLayout_shadow_corner_radius, 0);;
            topLeftRadius = typedArray.getDimension(R.styleable.ShadowFrameLayout_shadow_corner_top_left_radius, INVALID_VALUE);
            topRightRadius = typedArray.getDimension(R.styleable.ShadowFrameLayout_shadow_corner_top_right_radius, INVALID_VALUE);
            bottomRightRadius = typedArray.getDimension(R.styleable.ShadowFrameLayout_shadow_corner_bottom_right_radius, INVALID_VALUE);
            bottomLeftRadius = typedArray.getDimension(R.styleable.ShadowFrameLayout_shadow_corner_bottom_left_radius, INVALID_VALUE);
            ...
            int blurRadius = typedArray.getDimensionPixelSize(R.styleable.ShadowFrameLayout_shadow_blur_radius, 0);
            int shadowOffsetX = typedArray.getDimensionPixelSize(R.styleable.ShadowFrameLayout_shadow_offset_x, 0);
            int shadowOffsetY = typedArray.getDimensionPixelSize(R.styleable.ShadowFrameLayout_shadow_offset_y, 0);
            int shadowColor = typedArray.getColor(R.styleable.ShadowFrameLayout_shadow_color, Color.BLACK);
            //保证数值合法
            if (blurRadius < 0){
                blurRadius = 0;
            }
